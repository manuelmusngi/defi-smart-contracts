...
interface IERC20 {
    ...
    // to spend on behalf of `owner` through {transferFrom}. Default is Zero. 
    // TValue changes when {approve} or {transferFrom} are called.
    ...
    // Returns a boolean value indicating whether the operation succeeded.
    //
    // NOTE: Be attentive that changing an allowance with this method brings 
    ...
    // condition is to first reduce the spender's allowance to 0 and set the desired 
    // value afterwards:
    // 
    ...
    // `amount` is deducted from the caller's allowance balance.
    //  Returns a boolean value indicating whether the operation succeeded.
    //
    // Emits a {Transfer} event.
    ...
    // another (`to`). Note `value` may be zero.
